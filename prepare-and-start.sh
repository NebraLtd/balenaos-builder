...
if [ -d "${INSTALL_DIR}/balena-yocto-scripts" ]; then
    sudo -H -u builder -g builder "${INSTALL_DIR}/balena-yocto-scripts/build/barys" "-m" "$BASE_BOARD" "-l" "--bitbake-args" "-k -DDD -v" &
else
    sudo -H -u builder -g builder "${INSTALL_DIR}/resin-yocto-scripts/build/barys" "-m" "$BASE_BOARD" "-l"  "--bitbake-args" "-k -DDD -v" &
fi
